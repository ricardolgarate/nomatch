...
import { getOrderStats, getAllOrders, updateOrderStatus, Order } from '../firebase/orders';
import { LogOut, Save, Package, TrendingUp, ShoppingCart, Eye, Users, CreditCard, XCircle, CheckCircle2, Mail, Calendar, Tag, Plus, Trash2 } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const loadCoupons = async () => {
    try {
      const listCouponsFunc = httpsCallable(functions, 'listCoupons');
      const result = await listCouponsFunc();
      const data = result.data as { coupons: any[] };
  ...
  const handleCreateCoupon = async () => {
    ...
    try {
      setSaving('coupon');
      const createCouponFunc = httpsCallable(functions, 'createCoupon');
      
      await createCouponFunc({
        code: couponForm.code,
        type: couponForm.type,
        value: parseFloat(couponForm.value),
        maxRedemptions: couponForm.maxRedemptions ? parseInt(couponForm.maxRedemptions) : undefined,
        expiresAt: couponForm.expiresAt ? new Date(couponForm.expiresAt).getTime() : undefined,
      });
  ...
  const handleDeleteCoupon = async (couponId: string) => {
    ...
    try {
      const deleteCouponFunc = httpsCallable(functions, 'deleteCoupon');
      await deleteCouponFunc({ couponId });
      await loadCoupons();
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">NoMatch Store - Management</p>
              </div>
            </div>
            <button
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="bg-white rounded-xl shadow-lg p-6 border border-green-100">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-green-600">{orderStats.successfulOrders}</p>
                    </div>
                ...
                <div className="bg-white rounded-xl shadow-lg p-6 border border-red-100">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-red-600">{orderStats.failedOrders}</p>
                    </div>
                ...
                <div className="bg-white rounded-xl shadow-lg p-6 border border-blue-100">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-blue-600">${orderStats.totalRevenue.toFixed(2)}</p>
                    </div>
                      ...
                      <tr key={order.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <select
                            ...
                            onChange={(e) => updateOrderStatus(order.id, undefined, e.target.value as Order['fulfillmentStatus'])}
        ...
        {activeTab === 'coupons' && (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-serif font-semibold text-gray-900">Discount Coupons</h2>
              <button
              ...
              <div className="bg-white rounded-2xl shadow-xl p-8 border border-pink-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      {couponForm.type === 'amount' && (
                        <span className="absolute left-4 top-3 text-gray-500">$</span>
                      )}
                      <input
                        type="number"
                        placeholder={couponForm.type === 'percent' ? '10' : '20'}
                        value={couponForm.value}
                        onChange={(e) => setCouponForm({ ...couponForm, value: e.target.value })}
                        className={`w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 ${
                          couponForm.type === 'amount' ? 'pl-8' : ''
                        }`}
                      />
                    </div>
                    <p className="text-xs text-gray-500 mt-1">
                      {couponForm.type === 'percent' ? 'Enter number (e.g., 10 for 10% off)' : 'Enter dollar amount (e.g., 20 for $20 off)'}
                    </p>
                  </div>
                ...
                <div className="flex gap-4 mt-6">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors font-medium disabled:opacity-50"
                  >
                ...
                {coupons.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {coupons.map((coupon) => (
                      <div
                        key={coupon.id}
                        className="border-2 border-purple-100 rounded-xl p-5 hover:border-purple-300 hover:shadow-lg transition-all bg-gradient-to-br from-white to-purple-50/30"
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <Tag className="w-5 h-5 text-purple-600" />
                              <h4 className="font-bold text-lg text-gray-900">{coupon.id}</h4>
                            </div>
                        ...
                        <div className="space-y-1 text-sm text-gray-600">
                          <div className="flex justify-between">
                            <span>Status:</span>
                            <span className={`font-semibold ${coupon.valid ? 'text-green-600' : 'text-red-600'}`}>
                              {coupon.valid ? 'Active' : 'Expired'}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span>Used:</span>
                            <span className="font-semibold">{coupon.times_redeemed || 0} times</span>
                          </div>
                          {coupon.max_redemptions && (
                            <div className="flex justify-between">
                              <span>Max uses:</span>
                              <span className="font-semibold">{coupon.max_redemptions}</span>
                            </div>
                          )}
                          {coupon.redeem_by && (
                            <div className="flex justify-between">
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-200">
                          <button
                            ...
                            className="w-full px-4 py-2 bg-purple-50 text-purple-600 rounded-lg hover:bg-purple-100 transition-colors font-medium text-sm"
                          >
                            Copy Code
                          </button>
                        </div>
